...
class FolderWatcher:
    ...
    def start(self):
        ...
        for f in self.input_dir.glob("*.[wW][aA][vV]"):
            self._known_files.add(f.name)
        if self._known_files:
            msg = f"Found {len(self._known_files)} existing WAV file(s), skipped"
            logger.info(msg)
            self._log(msg)
    ...
    def _poll_loop(self):
        """Poll input directory for new WAV files."""
        while not self._stop_event.is_set():
            try:
                # Case-insensitive match for .wav/.WAV on WSL
                for f in sorted(self.input_dir.glob("*.[wW][aA][vV]")):
